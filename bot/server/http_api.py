...
class AlertAPI:
    ...
    def _check_auth(self, request: web.Request) -> Tuple[bool, str]:
        ...
        legacy_key = str(getattr(Config, "HTTP_API_KEY", "")).strip()
        allow_legacy = bool(getattr(Config, "ALLOW_LEGACY_API_KEY", False))
        if allow_legacy and legacy_key and hmac.compare_digest(supplied_token, legacy_key):
            logger.warning("Agent %s using legacy key", agent_id)
            return True, agent_id
    ...
    async def _read_json(self, request: web.Request) -> Dict[str, Any]:
        ...
        if not isinstance(payload, dict):
            raise web.HTTPBadRequest(text="JSON must be object")
        return payload
    ...
    async def handle_alert(self, request: web.Request) -> web.Response:
        ...
        payload = self._validate_alert(payload, agent_id)
    ...
    async def handle_alert_image(self, request: web.Request) -> web.Response:
        valid, agent_id = self._check_auth(request)
        if not valid:
            return web.json_response({"error": "Unauthorized"}, status=401)
    ...
    async def handle_heartbeat(self, request: web.Request) -> web.Response:
        ...
        if not isinstance(device_info, dict):
            return web.json_response({"error": "device_info must be object"}, status=400)

        self.db.register_agent(agent_id, device_info)
    ...
    async def handle_commands(self, request: web.Request) -> web.Response:
        ...
        commands = self.db.get_pending_commands(agent_id)
        return web.json_response({"commands": commands or []})

    async def handle_health(self, request: web.Request) -> web.Response:
        return web.json_response({"status": "healthy", "service": "mobile-security-monitor"})

    async def handle_root(self, request: web.Request) -> web.Response:
        return web.json_response({"service": "Mobile Security Monitor", "version": "1.2.0"})

    def _validate_alert(self, payload, authenticated_agent_id):
        title = str(payload.get("title", "Security Alert")).strip()
        ...
        if not isinstance(device_info, dict):
            raise ValueError("device_info must be object")
        if not timestamp:
            timestamp = datetime.now(timezone.utc).isoformat()
    ...
    async def _send_discord_alert(self, payload):
        channel_id = int(getattr(Config, "ALERT_CHANNEL_ID", 0))
        if not channel_id:
            return
        channel = self.bot.get_channel(channel_id)
        if channel is None:
            channel = await self.bot.fetch_channel(channel_id)
        await channel.send(embed=self._build_embed(payload))

    async def _send_discord_image(self, payload, image_data):
        channel_id = int(getattr(Config, "ALERT_CHANNEL_ID", 0))
        if not channel_id:
            logger.warning("Missing ALERT_CHANNEL_ID")
            return
        channel = self.bot.get_channel(channel_id)
        if channel is None:
            channel = await self.bot.fetch_channel(channel_id)
        ...
        if image_data:
            file = discord.File(io.BytesIO(image_data), filename="camera.jpg")
    ...
    def _build_embed(self, alert):
        ...
        colors = {"CRITICAL": 0xFF0000, "HIGH": 0xFF6600, "MEDIUM": 0xFFAA00, "LOW": 0x00AA00}
        emojis = {"CRITICAL": "🔴", "HIGH": "🟠", "MEDIUM": "🟡", "LOW": "🟢"}

        embed = discord.Embed(
            title=f"{emojis.get(severity, '⚪')} {alert.get('title', 'Alert')}",
            description=alert.get("description", "")[:4096],
            color=colors.get(severity, 0x3498DB),
        )
        embed.add_field(name="Severity", value=severity, inline=True)
        embed.add_field(name="Agent", value=str(alert.get("agent_id", "Unknown")), inline=True)
        embed.add_field(name="Time", value=str(alert.get("timestamp", ""))[:19], inline=True)

        details = alert.get("details", {})
        if isinstance(details, dict):
            for key, value in list(details.items())[:3]:
                embed.add_field(name=str(key).title(), value=str(value)[:1024], inline=True)

        embed.set_footer(text="Mobile Security Monitor")
        return embed
    ...
    async def start(self):
        ...
        await site.start()
        logger.info("HTTP API running %s:%s", self.bot.http_host, self.bot.http_port)
